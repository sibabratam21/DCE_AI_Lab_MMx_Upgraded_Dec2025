...
import { ModelRun, ModelLens, ModelConsistency } from '../types';
...
interface DualModelViewProps {
  ...
  onRecalibrate: (selectedChannels?: string[], updatedParams?: any) => void;
  onModelsUpdated?: (newModels: ModelRun[]) => void;
...
export const DualModelView: React.FC<DualModelViewProps> = (props) => {
  ...
  const handleSetActiveModelCustomer = (id: string | null) => {
    props.onSetActiveModel(id);
  };

  const handleSetActiveModelGeo = (id: string | null) => {
    // When selecting from GEO lens, map back to CUSTOMER ID
    if (id) {
      const baseId = id.replace('_GEO', '');
      props.onSetActiveModel(baseId);
    } else {
      props.onSetActiveModel(null);
    }
  };
        ...
        <div className="pb-3 pt-2">
          {activeTab === 'customer' && (
            <p className="text-xs text-gray-600">
              Customer-level model captures individual behavior and direct response effects
            </p>
          )}
          {activeTab === 'geo' && (
            <p className="text-xs text-gray-600">
              Geographic model captures market-level effects and broad reach channels (TV, brand)
            </p>
          )}
          {activeTab === 'consistency' && (
            <p className="text-xs text-gray-600">
              Compare CUSTOMER vs GEO perspectives to validate model assumptions and identify optimal modeling approach
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden">
        {activeTab === 'customer' && (
          <ModelingView
            ...
            onSetActiveModel={handleSetActiveModelCustomer}
          />
        )}
        ...
        {activeTab === 'geo' && (
          <ModelingView
            ...
            activeModelId={geoActiveModel?.id || null}
            onSetActiveModel={handleSetActiveModelGeo}
          />
        )}
